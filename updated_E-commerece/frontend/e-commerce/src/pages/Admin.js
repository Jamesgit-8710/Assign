...
import { del } from "../redux/slices/user.slice";
...
const Admin = () => {
  ...
  const handleOk = (e) => {
    if (name !== "" && price !== "" && qty !== "" && des !== "" && cat !== "") {
      if (files.length === 4) {
        const res = axios.post(`${process.env.REACT_APP_PATH}addProduct`, {
          name: name,
          price: price,
          qty: qty,
          des: des,
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      const res2 = await axios.post(`${process.env.REACT_APP_PATH}getUser`, {
        id: id,
      });
