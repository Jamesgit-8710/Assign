...
const Signup = ({ set, val, sent }) => {
  ...
  const signupWithemailandpass = (user, pass) => {
    ...
    createUserWithEmailAndPassword(auth, user, pass)
      .then(async (userCredential) => {
        ...
        const res = await axios.post(`${process.env.REACT_APP_PATH}user`, {
          user,
          pass,
          val,
          date: Date().toString(),
  ...
  const signupWithPhone = async (user, pass) => {
    ...
    const res = await axios.post(`${process.env.REACT_APP_PATH}exist`, { user });
    ...
    if (res.data) {
      ...
      const res2 = await axios.post(`${process.env.REACT_APP_PATH}user`, {
        user,
        pass,
        val,
        date: Date().toString(),
  ...
  const logInGoogle = async () => {
    await signInWithPopup(auth, provider)
      .then(async (result) => {
        ...
        const res = await axios.post(`${process.env.REACT_APP_PATH}exist`, {
          user: user.email,
        });
