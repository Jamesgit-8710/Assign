...
import { storage } from "../config/firbase.auth";
...
const Profile = ({ userData }) => {
  ...
  const onFinish = async () => {
    if (pass !== "") {
      ...
      const res = await axios.post(`${process.env.REACT_APP_PATH}updateUser`, {
        id: id,
        data: { password: pass },
      });
  ...
  const setImg = async (e) => { 
    if (e.target.value !== "") {
      ...
      await getDownloadURL(ref(storage, "images/" + imgName))
        ...
        const res = await axios.post(`${process.env.REACT_APP_PATH}setImg`, {
        id: id,
        data: imgUrl,
      });
  ...
  useEffect(() => {
    const call = async() => {
        const res = await axios.post(`${process.env.REACT_APP_PATH}getImg`, {
        id: id
    });
