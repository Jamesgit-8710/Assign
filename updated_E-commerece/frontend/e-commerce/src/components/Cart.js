...
const Cart = () => {
  ...
  const handleOk = async(e) => {
    ...
    if (address.trim() !== "" && value !== 0) {
      if (value === 1) {
        ...
        const res = await axios.post(`${process.env.REACT_APP_PATH}addOrder`, {data: data,id: id,status: 'd',payMethod: "cod",address: address});

        const res2 = await axios.post(`${process.env.REACT_APP_PATH}clearCart`, {myId: id});
        ...
        const res = await axios.post(`${process.env.REACT_APP_PATH}addOrder`, {data: data,id: id,status: 'd',payMethod: "card",address: address});
  ...
  useEffect(() => {
    const call = async () => {
      const res = await axios.post(`${process.env.REACT_APP_PATH}cartData`, {
        id: id,
      });

      setData(res.data);

      const res2 = await axios.post(`${process.env.REACT_APP_PATH}totalamount`, {
        id: id,
      });
      ...
      const res3 = await axios.post(`${process.env.REACT_APP_PATH}checkExist`, {
        myId: id,
      });
