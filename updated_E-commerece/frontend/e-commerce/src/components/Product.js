...
const Product = ({ item, show, set, setU, d }) => {
  ...
  const check = async () => {
    const res = await axios.post(`${process.env.REACT_APP_PATH}checkCart`, {
      id: item._id,
      myId: id,
    });
  ...
  const handleOk = async (e) => {
    const res = await axios.post(`${process.env.REACT_APP_PATH}update`, {
      id: item._id,
  ...
  const rmv = (e) => {
    const res = axios.post(`${process.env.REACT_APP_PATH}delete`, { id: item._id });
    messageApi.open({
      key,
      type: "success",
  ...
  const add = async (e) => {
    e.stopPropagation();
    setU(e)
    const res = await axios.post(`${process.env.REACT_APP_PATH}cart`, {
      id: id,
      data: {
        itemId: item._id,
        count: 1,
