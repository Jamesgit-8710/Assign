...
const Item = ({ item, awoke, show}) => {
  ...
  const rmv = async (e) => {
    const res = await axios.post(`${process.env.REACT_APP_PATH}deleteItem`, {
      id: id,
      itemId: item.itemId,
    });
  ...
  const cancel = async() => {
    const res = await axios.post(`${process.env.REACT_APP_PATH}updateOrder`, {
      id: item._id,
      data: "c",
    });
  ...
  const track = async () => {
    setVis("flex");
    const res = await axios.post(`${process.env.REACT_APP_PATH}getTrack`, {
      id: item._id,
    });
  ...
  useEffect(() => {
    const call = async () => {
      const res = await axios.post(`${process.env.REACT_APP_PATH}product`, {
        id: item.itemId,
      });
